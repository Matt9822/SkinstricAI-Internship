      ...
      <div className="flex-col h-[93vh] w-full">
        ...
        <div className="flex justify-between w-full ">
          <ul className="flex-col ml-10 w-[400]">
            <li onClick={() => setSelected("race")} className={`${selected === "race" ? "bg-black text-white" : "bg-gray-100"} flex items-center py-8 px-2 border-t-1 cursor-pointer ease-in-out duration-400`}>
              <p className="mr-1 font-[600] text-[15px] uppercase">race:</p>
              {sortedRaceGroup ? <p>{sortedRaceGroup[0][0]}</p> : <p>N/A</p>}
            </li>
            <li onClick={() => setSelected("age")} className={`${selected === "age" ? "bg-black text-white" : "bg-gray-100"} flex items-center my-5 py-8 px-2 border-t-1 cursor-pointer ease-in-out duration-400`}>
              <p className="mr-1 font-[600] text-[15px] uppercase">age:</p>
              {sortedAgeGroup ? <p>{sortedAgeGroup[0][0]}</p> : <p>N/A</p>}
            </li>
            <li onClick={() => setSelected("gender")} className={`${selected === "gender" ? "bg-black text-white" : "bg-gray-100"} flex items-center py-8 px-2 border-t-1 cursor-pointer ease-in-out duration-400`}>
              <p className="mr-1 font-[600] text-[15px] uppercase">sex:</p>
              {sortedGenderGroup ? <p>{sortedGenderGroup[0][0]}</p> : <p>N/A</p>}
            </li>
          </ul>
          <div className="flex bg-gray-100 mx-10 w-full">
            <div>
              <p>center</p>
            </div>
            <div></div>
          </div>
          <div className="flex-col mr-10 bg-gray-100 w-[450]">
            <HandleSelection sortedAge={sortedAgeGroup} sortedRace={sortedRaceGroup} sortedGender={sortedGenderGroup} setSelect={selected}/>
          </div>
        </div>
      </div>
    );
